...
public final class Xmls {
    ...
    public static String propertyFill(String filePathOrContent) {
        ...
        if (properties != null && properties.length > 0) {
            for (String property : properties) {
                System.out.println(String.format("${%1$s}", property));
                System.out.println(Property.get(property));
                filePathOrContent = filePathOrContent.replace(String.format("${%1$s}", property), Property.get(property));
                // String propertyValue = System.getProperty(property);
                // filePathOrContent = Strings.isNotNullOrEmpty(propertyValue) ? filePathOrContent.replace(String.format("${%1$s}", property), propertyValue) : filePathOrContent;
            }
        }
        return filePathOrContent;
    }
     ...
     * @param item
     */
    public static void include(Node includeNode, Node item) {
        try {
